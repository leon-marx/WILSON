...
class Trainer:
    ...
    def load_step_ckpt(self, path):
        ...
        if osp.exists(path):
            ...
            step_checkpoint = torch.load(path, map_location="cpu", weights_only=False, pickle_module=pickle)
    ...
    def load_ckpt(self, path):
        ...
        pickle.load = partial(pickle.load, encoding="latin1")
        pickle.Unpickler = partial(pickle.Unpickler, encoding="latin1")
        checkpoint = torch.load(opts.ckpt, map_location="cpu", weights_only=False, pickle_module=pickle)
